...
function initCursor() {
    ...
    if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) {
        ...
        const hoverElements = document.querySelectorAll('a, button, .portfolio-item, .video-item, .style-card');
...
const AgendamentoForm = {
    ...
    init() {
        ...
        // Atualizar horários inicialmente
    ...
    atualizarHorariosDisponiveis() {
        ...
        // Se não houver horários disponíveis para hoje
        if (horariosDisponiveis === 0 && ehHoje) {
            const option = document.createElement('option');
            option.value = '';
            option.textContent = 'Sem horários disponíveis hoje';
    ...
    async handleSubmit(e) {
        ...
        // Validações
        if (!this.isValidDate(formData.data)) {
            alert('A data deve ser hoje ou uma data futura.');
            this.elements.data.focus();
            return;
        }
        
        if (!this.isValidHora(formData.hora)) {
            alert('Por favor, selecione um horário válido (de 30 em 30 minutos, entre 10h e 20h)');
            this.elements.hora.focus();
            return;
        }
...
console.log('Jean Tattoo - Site carregado com sucesso!');
console.log('Imagens no portfólio:', CONFIG.portfolioImagens.length);
console.log('Vídeos configurados:', CONFIG.videos.length);
console.log('Efeitos 3D: Ativados!');
console.log('Mega Upgrade: Ativado!');
console.log('Novas funcionalidades: Bottom Nav, Parallax, Animações, Swipe!');
